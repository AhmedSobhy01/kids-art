...
bool CHexagon::Validate(Point c) {
	return (c.y - hexagonSize / 2 * sqrt(3) - FigGfxInfo.BorderWidth/2) > UI.ToolBarHeight && (c.y + hexagonSize / 2 * sqrt(3) + FigGfxInfo.BorderWidth / 2) <= (UI.height - UI.StatusBarHeight);
}